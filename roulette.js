...
function startRoulette(caseId) {
    ...
    tg.CloudStorage.getItem('userBalance', (err, val) => {
        ...
        setTimeout(() => {
            ...
            document.getElementById('won-skin-img').src = window.currentWinningSkin.img;
            document.getElementById('won-skin-name').innerText = window.currentWinningSkin.name;
            
            const priceEl = document.getElementById('won-skin-price');
            if (priceEl) {
                priceEl.innerHTML = `<img src="img/nav_diamond.png" style="width:16px; vertical-align:middle;"> ${window.currentWinningSkin.price} COIN`;
            }
            
            addToInventory(window.currentWinningSkin);
...
function sellWonSkin() {
    if (!window.currentWinningSkin) return;
    updateBalance(window.currentWinningSkin.price);
